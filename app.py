...
Módulo 3: Versão Estabilizada e Otimizada para Nuvem (Linux/Streamlit Cloud).
"""

import streamlit as str_app
from datetime import datetime

# Importações usando os módulos da pasta engine
...
# Inicialização da memória temporária (Session State)
...
with col_cadastro:
    str_app.header("📋 Ficha de Avaliação Clínica")
...
def formular_dieta_estabilizada(num_etapa: int, peso_alvo: float, nem_meta: float, total_alimentos: int):
    ...
    for i in range(total_alimentos):
        ...
        with col_nome:
            ...
            if nomes:
                nom_sel = str_app.selectbox("Selecionar Produto:", nomes, key=f"nome_e{num_etapa}_a{i}")
...
nem_t1 = calcular_nem(especie, is_castrado == 1, peso_t1, fator_escolhido)
